...
import {CarApi, HPriceListApi, MCarReservationApi, RACServiceApi} from '../shared/sdk/services/custom';
import {Car, LoopBackConfig, MCarReservation, RACService} from '../shared/sdk';
import {API_VERSION} from '../shared/baseurl';
import {ToastrService} from 'ngx-toastr';

@Component({
  selector: 'app-rac-report-section',
  templateUrl: './rac-report-section.component.html',
  styleUrls: ['./rac-report-section.component.scss']
})
export class RacReportSectionComponent implements OnInit {
  ...
  private reservations_lastdays: MCarReservation[] = [];
  private reservations_lastweeks: MCarReservation[] = [];
  private reservations_lastmonths: MCarReservation[] = [];
  @Input()
  profile: RACService;
  ...
  ngOnInit() {
    ...
    this.carApi.find({where: {rACServiceId: this.profile.id}}).subscribe((cars: Car[]) => {
      ...
      cars.forEach((car) => {
        this.carRatings.push(car);
        this.mCarReservationApi.find({where: {carId: car.id}}).subscribe((reservations: MCarReservation[]) => {
